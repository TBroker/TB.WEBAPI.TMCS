...
namespace TB.WEBAPI.TMCS.Controllers
{
    ...
    public class MotorController(IApiClientService apiClientService) : ControllerBase
    {
        ...
        {
            var response = await _apiClientService.PostAsync<MotorSubmitRequest, IEnumerable<SubmitMotorResponse>>("/apiMotorVMI/ApplMotorVMI", request);
        ...
        {
            var response = await _apiClientService.PostAsync<MotorUploadFileRequest, IEnumerable<ApiResponseDtoCoreSystem>>("/apiMotorVMI/AttachFileVol", request);
            return response.Code == 200 ? Ok(response) : BadRequest(response);
        }
    }
}